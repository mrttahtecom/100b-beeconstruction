...
if ( function_exists( 'edifice_elementor_is_preview' ) && edifice_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			edifice_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	edifice_new_main_query(
		array(
			'post_type'      => edifice_get_theme_option( 'post_type' ),
			'category'       => edifice_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'edifice_filter_get_template_part', edifice_blog_archive_get_template() ) );
}
