...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			edifice_create_observer( 'edifice-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				edifice_gutenberg_editor_init();
			} );
		} else {
		 	edifice_gutenberg_editor_init();
		}
	...
	function edifice_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'edifice_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'edifice_inited' ) ) {
	...
	// Init on page load
	function edifice_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = edifice_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + EDIFICE_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + EDIFICE_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in EDIFICE_STORAGE['override_classes'] ) {
				$obj.addClass( EDIFICE_STORAGE['override_classes'][i].replace( '%s', EDIFICE_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( edifice_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && EDIFICE_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'edifice_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			edifice_remove_observer( 'edifice-check-editor-styles-wrapper' );
			edifice_create_observer( 'edifice-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'edifice_inited' );
	}
} );
