...
jQuery( document ).ready(
	function() {
		...
		edifice_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="edifice_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.edifice_post_editor' );
						if ( link.length > 0 ) {
							edifice_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.edifice_post_editor', function(e) {
						edifice_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('edifice_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function edifice_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
