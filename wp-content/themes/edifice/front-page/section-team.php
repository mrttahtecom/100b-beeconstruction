<div class="front_page_section front_page_section_team<?php
	$edifice_scheme = edifice_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $edifice_scheme ) && ! edifice_is_inherit( $edifice_scheme ) ) {
		echo ' scheme_' . esc_attr( $edifice_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( edifice_get_theme_option( 'front_page_team_paddings' ) );
	if ( edifice_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$edifice_css      = '';
		$edifice_bg_image = edifice_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$edifice_anchor_icon = edifice_get_theme_option( 'front_page_team_anchor_icon' );
	$edifice_anchor_text = edifice_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( edifice_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$edifice_css      = '';
			$edifice_bg_mask  = edifice_get_theme_option( 'front_page_team_bg_mask' );
			$edifice_bg_color_type = edifice_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $edifice_bg_color_type ) {
				$edifice_bg_color = edifice_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$edifice_caption = edifice_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $edifice_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $edifice_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $edifice_caption, 'edifice_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$edifice_description = edifice_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $edifice_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $edifice_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $edifice_description ), 'edifice_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! edifice_exists_trx_addons() ) {
						...
						edifice_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
