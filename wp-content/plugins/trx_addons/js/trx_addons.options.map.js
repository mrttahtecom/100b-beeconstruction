jQuery(document).ready(function() {
	...
	var MapObject = function(map_wrapper) {
		this.newMarkerApi = typeof google.maps.marker != 'undefined' && typeof google.maps.marker.AdvancedMarkerElement != 'undefined';
	...
	MapObject.prototype = {
		...
		initMapElements: function() {
			...
			if ( this.map_type == 'google' ) {
				...
				if ( this.newMarkerApi ) {
					mapInit['mapId'] = this.map_id;
				}
				this.map = new google.maps.Map( this.map_wrapper.get(0), mapInit );
				// Add marker
				var markerInit = {
					position: center,
					map: this.map,
				...
				if ( this.newMarkerApi ) {
					markerInit['gmpDraggable'] = true;
				} else {
					markerInit['draggable'] = true;
				}
				this.marker = new google.maps.Marker( markerInit );
				// Add geocoder
